...
import "./about.css";
const Cyberbutton = ({ InputText }) => {
    return (
        <div className="grid min-h-[200px] place-content-center bg-neutral-900 p-4">
            <EncryptButton inputtext={InputText} />
        </div>
    );
};
const EncryptButton = ({ inputtext }) => {
    const intervalRef = useRef(null);
    const TARGET_TEXT = inputtext;
    ...
    return (
        ...
        >
            <div className="relative z-10 flex items-center gap-2">
                <span className={`text-white   border-2 m-3  rounded-4 ${text == 'X' ? 'p-5' : 'p-3'}`} >{text}</span>
            </div>
            <motion.span
                initial={{
                    y: "100%",
